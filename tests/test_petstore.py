...
class TestFlaskHandlerPet(TestCase):
    """Tests for `flask_handler` using the pet model."""
    ...
    def test_post(self):
        ...
        pet_post = Object(
            properties={
                ...
                Property("photoUrls", name="photo_urls", required=True): Array(
                    items=String()
                ),
        ...
        @flask_handler(handler=pet_post.assemble(), json=flask_json)
        def pet(
            name: str,
            photo_urls: list[str],
            id_: Optional[int] = None,
